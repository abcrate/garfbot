    ...
    async def scan(self, message, user, guild, query):
        ...
        if query.startswith("garfping "):
            try:
                ...
                embed = discord.Embed(title="Ping result: {target}", color=0x4D4D4D, description=f"```{result.stdout}```")
        ...
        if query.startswith("garfdns "):
            try:
                ...
                embed = discord.Embed(title="NSLookup result: {target}", color=0x4D4D4D, description=f"```{result.stdout}```")
                await message.channel.send(embed=embed)
            except Exception as e:
                await message.channel.send(f"`GarfBot Error: {str(e)}`")
        ...
        if query.startswith("garfhack "):
            try:
                ...
                embed = discord.Embed(title="Nmap scan result: {target}", color=0x4D4D4D, description=f"```{result.stdout}```")
